...
'''Run special tests for INSN_CNT CSR functionality

We expect tests below ./special. Each test is defined by two files (in the same
...
def find_special_tests() -> List[Tuple[str, str]]:
    '''Find all tests below ./special (relative to this file)

    Returns (asm, expected) pairs, with the paths to the assembly file and
    expected values.

    '''
    root = os.path.join(os.path.dirname(__file__), 'special')
    ret = []
...
def test_special(tmpdir: py.path.local,
                 asm_file: str,
...
def pytest_generate_tests(metafunc: Any) -> None:
    if metafunc.function is test_special:
        tests = find_special_tests()
